...
I18n.register('zh-CN', {
  ...
  'restart.currentVersion': '当前版本: v{version}',
  'restart.dataWarning': '如果您的部署使用可变镜像标签（例如 "latest"），重启可能会更改运行版本。版本降级可能导致数据丢失。请将镜像固定到特定版本标签以避免此问题。',
  'restart.versionChanged': '警告: 重启后 IronClaw 版本已更改（之前为 v{oldVersion}，现在为 v{newVersion}）。如果这不是有意为之，请检查您的部署配置。',
