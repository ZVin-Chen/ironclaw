...
function applyEngineModeToTabs() {
  document.querySelectorAll('.tab-bar [data-v2-only]').forEach(function(el) {
    el.style.display = engineV2 ? '' : 'none';
  });
...
// Override openMissionDetail and openEngineThread to render into cr-detail.
...
// Legacy loadMissions (kept for v1 missions tab if still referenced).
...
// Legacy openMissionDetail/openEngineThread/closeMissionDetail/renderMissionDetail
// removed — the control room versions above handle all mission/thread display.
...
function fetchGatewayStatus() {
  apiFetch('/api/gateway/status').then(function(data) {
    ...
    if (!engineModeApplied) {
      engineV2 = !!data.engine_v2_enabled;
